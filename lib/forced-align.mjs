...
function emissionShape(emission) {
  if (emission?.data && emission.numFrame && emission.numLabel) {
    return { numFrame: emission.numFrame, numLabel: emission.numLabel, data: emission.data };
  }
  return { numFrame: emission.length || 0, numLabel: emission[0]?.length || 0, data: null };
}
...
export function buildTrellis(emission, tokens, blankId = 0) {
  const { numFrame, numLabel, data } = emissionShape(emission);
  ...
  for (let t = 1; t < numFrame; t++) {
    cum += data ? data[t * numLabel + blankId] : emission[t][blankId];
    T[t * numTokens + 0] = cum;
  }
  // trellis[0, 1:] = -inf
  ...
  if (data) {
    for (let t = 0; t < numFrame - 1; t++) {
      const row = t * numLabel;
      const cur = t * numTokens;
      const next = (t + 1) * numTokens;
      const blank = data[row + blankId];
      for (let j = 1; j < numTokens; j++) {
        const stay = T[cur + j] + blank; // same token, blank frame
        const change = T[cur + j - 1] + data[row + tokens[j]]; // next token
        T[next + j] = stay > change ? stay : change;
      }
    }
  } else {
    for (let t = 0; t < numFrame - 1; t++) {
      ...
      const cur = t * numTokens;
      const next = (t + 1) * numTokens;
      const blank = row[blankId];
      for (let j = 1; j < numTokens; j++) {
        const stay = T[cur + j] + blank; // same token, blank frame
        const change = T[cur + j - 1] + row[tokens[j]]; // next token
        T[next + j] = stay > change ? stay : change;
      }
    }
  }
...
export function backtrack(trellis, emission, tokens, blankId = 0) {
  ...
  const initialBlank = data ? data[t * numLabel + blankId] : emission[t][blankId];
  const path = [{ tokenIndex: j, timeIndex: t, score: Math.exp(initialBlank) }];
  while (j > 0) {
    // t should always be > 0 here for a valid alignment.
    const row = data ? (t - 1) * numLabel : null;
    const pStay = data ? data[row + blankId] : emission[t - 1][blankId];
    const pChange = data ? data[row + tokens[j]] : emission[t - 1][tokens[j]];
  ...
  while (t > 0) {
    const pBlank = data ? data[(t - 1) * numLabel + blankId] : emission[t - 1][blankId];
    path.push({ tokenIndex: j, timeIndex: t - 1, score: Math.exp(pBlank) });
    t -= 1;
  }
  return path.reverse();
}
...
export function alignTokens(emission, tokens, { blankId = 0, separatorId = 1 } = {}) {
  const { numFrame } = emissionShape(emission);
